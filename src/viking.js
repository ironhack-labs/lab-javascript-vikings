...
class War {
  ...
  vikingAttack(){
    ...
    this.saxonArmy = this.saxonArmy.filter(soldier => {
      if(soldier.health > 0){return soldier}
    })

    if (randomSaxon.health < 0){return `A Saxon has died in combat`
    } else {return `A Saxon has received ${randomViking.strength} points of damage`}
  }
