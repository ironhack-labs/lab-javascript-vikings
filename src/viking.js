...
class War {
  ...
  vikingAttack() {
    ...
    const saxonDamage = this.saxonArmy[randomIndSax].receiveDamage(
      vikingAttack
    );
  ...
  saxonAttack() {
    ...
    let randomIndVik = Math.round(Math.random(this.vikingArmy.length - 1) * 10);
    const vikingDamage = this.vikingArmy[randomIndVik].receiveDamage(
      saxonAttack
    );
    const newVikingArmy = this.vikingArmy.filter(
      (viking) => viking.health <= 0
    );
