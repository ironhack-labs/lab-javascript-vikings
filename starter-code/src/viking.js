...
class War {
  ...
  vikingAttack(){
    ...
    // check Prof. Nick's way of doing it on Github shown during review
    let randomViking = Math.floor(Math.random() * this.vikingArmy.length);
    this.saxonArmy[randomSaxon].receiveDamage(this.vikingArmy[randomViking].strength); 
    if (this.saxonArmy[randomSaxon].health <= 0){
      this.saxonArmy = this.saxonArmy.splice(randomSaxon, randomSaxon-1);
      return `A Saxon has died in combat`;
    }
  }
  saxonAttack(){
    ...
    if (this.vikingArmy[randomViking].health <= 0){
      this.vikingArmy = this.vikingArmy.splice(randomViking, randomViking-1);
