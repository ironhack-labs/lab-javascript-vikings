...
class War {
  ...
  _randomSoldier(soldiers) {
    const rnd = Math.random() * (soldiers.length - 1);
    return soldiers[rnd];
  }

  vikingAttack() {
    const result = this._randomSoldier(this.saxonArmy).receiveDamage(
      this._randomSoldier(this.vikingArmy).strength
    );
  ...
  saxonAttack() {
    const result = this._randomSoldier(this.vikingArmy).receiveDamage(
      this._randomSoldier(this.saxonArmy).strength
    );
