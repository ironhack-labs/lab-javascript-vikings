...
var weapons = ['gun', 'hammer', 'granade', 'bazooka', 'knife', 'shotgun', 'sniper', 'assault rifle', 'sword', 'missile', 'revolver'];

var createViking = document.getElementById('createViking');
var createSaxon  = document.getElementById('createSaxon');
var attackViking = document.getElementById('attackViking');

var attackSaxon  = document.getElementById('attackSaxon');
var result       = document.getElementById('result');
var vikingDraw    = document.getElementById('vikingDraw');
var saxonDraw    = document.getElementById('saxonDraw');
...
//VIKING
function Viking(name, health, strength, randomWeapon, team) {
...
////SAXON
function Saxon(name, health, strength, randomWeapon, team) {
    Soldier.call(this, health, strength);
    this.name = name;
    this.strength = strength;
    this.randomWeapon = randomWeapon;
...
Saxon.prototype.receiveDamage = function(damage){
    ...
    if (this.health > 0) {
        ...
        return this.name + ' has died in act of combat from';
    }
};
...
//Method add viking
War.prototype.addViking = function(numberToCreate){    
    for (let i = 0; i < numberToCreate; i++) {
        var randomName   = names[Math.floor(Math.random()*names.length)];
        var randomWeapon = weapons[Math.floor(Math.random()*weapons.length)];
        var randomStrength = Math.floor(Math.random()*100);
        var viking = new Viking(randomName, 100, randomStrength, randomWeapon, 'viking');
        this.vikingArmy.push(viking);
    }
};

//Method add saxon
War.prototype.addSaxon = function(numberToCreate){    
    for (let i = 0; i < numberToCreate; i++) {
        var randomName   = names[Math.floor(Math.random()*names.length)];
        var randomWeapon = weapons[Math.floor(Math.random()*weapons.length)];
        var randomStrength = Math.floor(Math.random()*100);
        var saxon = new Saxon(randomName, 100, randomStrength, randomWeapon, 'saxon');
        this.saxonArmy.push(saxon);
    }};

    //Method fight
    var turn = 0;
    War.prototype.attack = function(numToAttack){    
        ...
        if (turn === 0) {
            var vikingUpdated = randomViking.receiveDamage(randomSaxon.strength) + ' from ' + randomSaxon.name + '(saxon)' + ' with a ' + randomSaxon.randomWeapon;
            console.log(turn, 'turn saxon');
                  
            this.vikingArmy = this.vikingArmy.filter(function(viking){
                return viking.health > 0; 
            });
            turn = 1;
            return vikingUpdated; 
        } else{
            var saxonUpdated = randomSaxon.receiveDamage(randomViking.strength) + ' from ' + randomViking.name + '(viking)' +  ' with a ' + randomViking.randomWeapon;;
            console.log(turn, 'turn viking');
    ...
    //Method status
    War.prototype.showStatus = function(partidaIniciada){            
        if (war.partidaIniciada) {
            if (this.saxonArmy.length === 0) {
                // clearInterval(intervalId);
                return "Vikings have won the war of the century!, SURVIVORS: " + ' (' + this.vikingArmy.length + ')';
            } else if(this.vikingArmy.length === 0){
                clearInterval(intervalId);
                return "Saxons have won the war of the century!: survivors: " + ' (' + this.saxonArmy.length + ')';   
            } else if(this.saxonArmy.length > this.vikingArmy.length){
                return 'Saxons are winning the war';
            } else if(this.saxonArmy.length < this.vikingArmy.length){
                return 'Vikings are winning the war';   
            } else if(this.saxonArmy.length === this.vikingArmy.length){
                return 'The war is even';   
            }
        } else{
            return 'war didnt start';
        }
    };
    ...
    var start = false;
    var turn  = 0;
    createViking.onclick = function(e){    
        war.addViking(80);
        vikingDraw.innerHTML += ' ('+ war.vikingArmy.length +' VIKINGS)';
        vikingDraw.style.fontSize = '12px';
        vikingDraw.style.color = 'red';
        start = true;
    };
    
    createSaxon.onclick = function(e){
        war.addSaxon(80); 
        saxonDraw.innerHTML += ' ('+ war.saxonArmy.length +' SAXONS)';
        saxonDraw.style.fontSize = '12px';
        saxonDraw.style.color = 'blue';
        start = true;
    };
   ...
   var intervalId =  setInterval(function(){
        if (start) {            
            if (turn === 0) {                    
                war.partidaIniciada = true;
                saxonDraw.innerText += '\n' + war.attack(3); 
                turn = 1;
            } else{
                war.partidaIniciada = true;
                vikingDraw.innerText +='\n' + war.attack(3);
                turn = 0;
            }
        }
    },10);
    ...
    setInterval(function(){
        result.innerText = war.showStatus();         
    },300);
