...
describe("War", () => {
  ...
  describe("getRandomSoldier() method", () => {
    it("should be declared", () => {
      expect(typeof war.getRandomSoldier).toBe("function");
    });

    it("should receive 1 argument (a soldiers array[])", () => {
      expect(war.getRandomSoldier.length).toEqual(1);
    });

    it("should return an object with a soldier and its index on army", () => {
      war.addViking(viking);

      expect(war.getRandomSoldier(war.vikingArmy)).toEqual(
  ...
  describe("getOpponents() method", () => {
    it("should be declared", () => {
      expect(typeof war.getOpponents).toBe("function");
    });

    it("should receive 1 argument (a type of soldier string)", () => {
      expect(war.getOpponents.length).toEqual(1);
    });

    it("should return an object with two opponent soldiers, their indexes on their armies, and the enemy army", () => {
      war.addViking(viking);
      war.addSaxon(saxon);

      expect(war.getOpponents("viking")).toEqual(
        jasmine.objectContaining({
          attacker: { index: 0, soldier: viking },
          enemy: { army: war.saxonArmy, index: 0, soldier: saxon },
        })
      );
    });
  });

  describe("doFigth() method", () => {
    it("should be declared", () => {
      expect(typeof war.doFight).toBe("function");
    });

    it("should receive 2 arguments (an attacker and enemy objects)", () => {
      expect(war.doFight.length).toEqual(2);
    });

    it("if viking attacks saxon it should return saxon damage string", () => {
      war.addViking(viking);
      war.addSaxon(saxon);

      const attacker = { index: 0, soldier: viking };
      const enemy = { army: war.saxonArmy, index: 0, soldier: saxon };

      const vikingDamageString = "A Saxon has died in combat";

      expect(war.doFight(attacker, enemy)).toEqual(vikingDamageString);
    });
  });

  describe("removeDead() method", () => {
    it("should be declared", () => {
      expect(typeof war.removeDead).toBe("function");
    });

    it("should receive 3 arguments (a soldier's health number, its army index, and its army array", () => {
      expect(war.removeDead.length).toEqual(3);
    });

    it("if a viking kills a saxon it should be removed from its army", () => {
      war.addSaxon(saxon);
      saxon.health = 0;
      war.removeDead(saxon.health, 0, war.saxonArmy)

      expect(war.saxonArmy.length).toEqual(0);
    });
  });

  describe("soldierAttack() method", () => {
    it("should be declared", () => {
      expect(typeof war.soldierAttack).toBe("function");
    });

    it("should receive 1 arguments (a soldier type string", () => {
      expect(war.soldierAttack.length).toEqual(1);
    });

    it("if a viking attacks a saxon it should return the saxon damage message", () => {
      war.addSaxon(saxon);
      war.addSaxon(viking);

      const attackMessage = "A Saxon has died in combat";
      
      expect(war.soldierAttack('viking')).toEqual(attackMessage);
    });
  });
